...
public class MenuBaseInfoController extends AbstractController {
	...
	public Result nav(@RequestParam("roleId") Long roleId) {
		...
		//List<MenuBaseInfoEntity> menuList = menuBaseInfoService.getUserMenuList(userId);
	...
	public Result list(@RequestBody MenuBaseInfoEntity menuBaseInfoEntity){
		Result result = new Result();
//		List<MenuBaseInfoEntity> menuList = menuBaseInfoService.selectList(menuBaseInfoEntity);
//		for(MenuBaseInfoEntity menuBaseInfoEntity : menuList){
//			MenuBaseInfoEntity parentMenuEntity = menuBaseInfoService.selectById(menuBaseInfoEntity.getParentId());
...
//		result = ResponseResult.success(menuList);
	...
	public Result select() {
		Result result = new Result();
		//查询列表数据
		List<MenuBaseInfoEntity> menuList = menuBaseInfoService.queryNotButtonList();
	...
	public Result info(@RequestBody SysMenuParam param){
		...
		if (StringUtil.isEmpty(param.getMenuId())){
			...
			MenuBaseInfoEntity menu = menuBaseInfoService.selectById(param.getMenuId());
	...
	public Result save(@RequestBody MenuBaseInfoEntity menu){
		...
		//数据校验
		verifyForm(menu);
//		result = menuBaseInfoService.insert(menu);
...
//		result = menuBaseInfoService.updateById(menu);
	...
	public Result delete(@RequestBody SysMenuParam param) {
		...
		if (StringUtil.isEmpty(param.getMenuId())){
			...
			//判断是否有子菜单或按钮
			List<MenuBaseInfoEntity> menuList = menuBaseInfoService.queryListParentId(menuId);
			if (menuList.size() > 0) {
...
//			MenuBaseInfoEntity parentMenu = menuBaseInfoService.selectById(menu.getParentId());
