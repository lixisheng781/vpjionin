...
public class ResponseResult extends TransactionAspectSupport {
     ...
     * 失败,带状态、描述、内容
     */
    public static <T> Result<T> failResult(ResultCodeEnum resultCode, String msg, T data) {
        TransactionInfo info = currentTransactionInfo();
        if (info != null && info.getTransactionStatus() != null) {
            logger.warn("事务异常回滚: " + msg);
            info.getTransactionStatus().setRollbackOnly();
     ...
     * 失败,带状态、描述、内容
     */
    public static <T> Result<T> failResult(ResultCodeEnum resultCode, String msg, T data, boolean rollback) {
        if (rollback) {
            TransactionInfo info = currentTransactionInfo();
            if (info != null && info.getTransactionStatus() != null) {
                logger.warn("事务异常回滚: " + msg);
