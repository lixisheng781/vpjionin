...
public final class HttpUtils {
    ...
    public final static class HttpRequest {
        ...
        /***
         * 执行请求
         * @return statusCode 状态，response 响应内容
         */
        public HttpUtilResponse execute() {
            try {
                return doService();
            } catch (Exception e) {
                throw new RuntimeException(e);
         ...
         * 设置SSL 证书
         */
        public HttpRequest setSslConnectionSocketFactory(SSLConnectionSocketFactory sslConnectionSocketFactory) {
         ...
         * @return statusCode 状态，response 响应内容
         */
        private HttpUtilResponse doService() throws Exception {
            try (CloseableHttpClient client = getHttpClient()) {
                HttpUriRequest request = getHttpUriRequest();
                // 执行请求
                HttpResponse response = client.execute(request);
                ...
                // 结果集
                if (statusCode == 200) {
                    return new HttpUtilResponse(statusCode, responseText, "请求成功");
                } else {
                    return new HttpUtilResponse(statusCode, responseText, "请求失败");
                }
            }
        }
        ...
        private Map<String, String> convertMap(Object obj) {
            ...
            if (obj instanceof Map) {
                ...
                IntStream.range(0, array.length).filter((i) -> i % 2 == 0).forEach((i) -> target.put(array[i].toString(), array[i + 1].toString()));
            }
            return target;
        }
        ...
        private CloseableHttpClient getHttpClient() throws Exception {
            ...
            if (url.startsWith("https")) {
                if (sslConnectionSocketFactory == null) {
                    sslConnectionSocketFactory = new SSLConnectionSocketFactory(new SSLContextBuilder().loadTrustMaterial(null, new TrustStrategy() {
                        // 信任所有
                        @Override
